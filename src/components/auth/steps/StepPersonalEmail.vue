...
watch(name, (newVal) => {
  if (newVal) isName.value = true;
});
watch(gender, (newVal) => {
  if (newVal) isGender.value = true;
});
watch(mobile, (newVal) => {
  if (newVal) isMobile.value = true;
});
...
async function saveData() {
  if (valid.value) {
    ...
    if (isMobile.value && isName.value && isGender.value) {
      try {
        const response = await http.post(`/gypsy/save-normal-user`, payload, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
        const data = response.data;
        logger.log("saveData email response:", data);
        ...
        logger.error("saveData email error:", error);
