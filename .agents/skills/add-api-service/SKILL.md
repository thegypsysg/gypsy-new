...
- Semua service **WAJIB** menggunakan `http` dari `src/api/http.js`.
- **JANGAN** gunakan `src/util/axios.js` — sudah **@deprecated**.
...
## Alternatif: useFetch (untuk komponen Vue)

Jika data hanya diperlukan di level komponen (bukan store), pertimbangkan `useFetch`:

```js
import { useFetch } from "@/composables/useFetch";

// Dengan caching otomatis (default TTL 60s)
const { data, loading, error, execute } = useFetch("/nama-endpoint");

// Tanpa caching
const { data } = useFetch("/nama-endpoint", { cache: false });
```

`useFetch` memiliki in-memory cache dan mengembalikan reactive state yang cocok untuk komponen.
