...
- Ganti `src/assets/header.webp` (banner utama homepage) dan `src/assets/images/banner-mobile.webp`.
- Ganti `src/assets/partner-*.webp` dengan gambar partner brand baru.
...
### 9. Security Headers — `public/.htaccess`

Review Content Security Policy (CSP) di `public/.htaccess`. Pastikan domain API baru sudah ditambahkan ke whitelist:

```apache
Header always set Content-Security-Policy "default-src 'self'; \
  connect-src 'self' https://api.domain-baru.com; \
  img-src 'self' data: https://api.domain-baru.com; \
  ..."
```
...
[ ] Gambar aset lama sudah diganti (banner, logo, background, partner images)
[ ] npm run optimize:images sudah dijalankan untuk gambar baru
[ ] CSP di public/.htaccess sudah disesuaikan dengan domain API baru
